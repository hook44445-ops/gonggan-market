-- ════════════════════════════════════════════════════════════════════
-- 061_admin_project_flow_no_comp_phone.sql
-- admin_project_flow_list — companies.phone 컬럼 부재 대응(모든 overload DROP 후 재생성).
--
-- 배경:
--   GPS 흐름관리 진입 시 "column comp.phone does not exist".
--   운영 DB public.companies 테이블에 phone 컬럼이 없는데 RPC 가 comp.phone 참조.
--
-- 수정(RPC 내부 SQL만):
--   · company json 생성부에서 'phone', comp.phone 제거.
--   · 검색 조건에서 comp.phone ilike ... 라인 제거.
--   · 그 외(055 기준): r.urgent→false, r.updated_at→r.created_at, b.selected 미사용,
--     'admin' sentinel 허용, 단계계산/조인/필터 동일.
--   · 잔존 버전 확실 제거 위해 동명 모든 overload 동적 DROP 후 단일 CREATE.
--   · companies 컬럼 추가 없음. 시그니처/ grant 유지. 조회 전용.
--
-- 검증(실행 후 0 이어야 함):
