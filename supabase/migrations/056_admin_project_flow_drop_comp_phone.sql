-- ════════════════════════════════════════════════════════════════════
-- 056_admin_project_flow_drop_comp_phone.sql
-- admin_project_flow_list — comp.phone 참조 제거(운영 companies.phone 없음).
--
-- 배경:
--   운영 DB public.companies 에 phone 컬럼이 없는데 admin_project_flow_list 가
--   comp.phone 을 참조해
--     column comp.phone does not exist
--   로 GPS 흐름관리가 실패했다.
--
-- 수정(055 정합본 기준, comp.phone 2곳만 제거):
--   · company json 에서 'phone', comp.phone 제거.
--   · 검색 조건에서 or comp.phone ilike ... 제거.
--   · 그 외 동일 — 시그니처/ grant/ notify 유지, r.urgent=false, b.selected 미사용,
--     r.updated_at 미참조(r.created_at). companies.phone 컬럼 추가 없음. 조회 전용.
--
-- 확실한 적용: 동명 overload 전부 DROP 후 단일 정의 재생성.
--
-- 검증(실행 후 0):
...
--
-- 멱등 · Supabase SQL Editor 에서 1회 실행(055 이후).
-- ════════════════════════════════════════════════════════════════════

set search_path = public, extensions;

-- 1) 동명 overload 전부 제거(시그니처 무관).
do $$
declare r record;
begin
  for r in
...
begin
  ...
  from (
    ...
    -- 선택 업체: 1순위 r.selected_company_id, 2순위 r.selected_bid_id 의 입찰 업체(선택 플래그 미사용).
    ...
    -- 선택 입찰: r.selected_bid_id 기준(선택 플래그 미사용).
    left join lateral (
