-- ════════════════════════════════════════════════════════════════════
-- 064_admin_project_flow_escrow_safe.sql
-- admin_project_flow_list — escrow 컬럼 부재(운영 스키마 불일치) 널-가드.
--
-- 배경:
--   "column esc.step2_paid_at does not exist" 등 escrow_payments 컬럼이 운영 DB에
--   여러 개 없을 수 있음(코드와 불일치). 한 컬럼씩 잡으면 반복되므로 일괄 처리.
--
-- 수정(기존 구조 유지, RPC 내부 SQL만):
--   · escrow 람다에 to_jsonb(e) as ej 추가.
--   · escrow json 의 확정 컬럼(id/transaction_status/created_at) 외 모든 필드를
--     esc.ej ->> '..' 로 추출 → 컬럼 없으면 null, 키/구조 유지.
--   · flow_stage 의 esc.step4_approved_at → (esc.ej ->> 'step4_approved_at').
--   · 프론트는 escrow.transaction_status/step4_approved_at/total_amount 만 사용(표시) →
--     text 추출이어도 동작 무방.
--   · 그 외(comp.phone/sb.period_days 제거, urgent false, r.created_at, b.selected 미사용,
--     'admin' sentinel) 063 과 동일. 새 컬럼/조인 추가 없음.
--
-- 검증(0 이어야 함):
--   select position('esc.step2_paid_at' in pg_get_functiondef(p.oid))
...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'flow_stage', case
        ...
        when esc.transaction_status = 'COMPLETED'
          or (esc.ej ->> 'step4_approved_at') is not null
      ...
      -- escrow_payments 운영 스키마가 코드와 달라 일부 컬럼이 없을 수 있으므로,
      -- 확정 컬럼(id/transaction_status/created_at) 외에는 to_jsonb(esc.ej)->>'..' 로
      -- 추출(없으면 null) — 구조/키 유지, column does not exist 차단.
      'escrow', case when esc.id is null then null else jsonb_build_object(
        'id', esc.id,
        'transaction_status', esc.transaction_status,
        'status',             esc.ej ->> 'status',
        'total_amount',       esc.ej ->> 'total_amount',
        'current_step',       esc.ej ->> 'current_step',
        'step1_deposited_at', esc.ej ->> 'step1_deposited_at',
        'step2_paid_at',      esc.ej ->> 'step2_paid_at',
        'photos_uploaded_at', esc.ej ->> 'photos_uploaded_at',
        'step3_approved_at',  esc.ej ->> 'step3_approved_at',
        'step3_disputed',     esc.ej ->> 'step3_disputed',
        'step4_approved_at',  esc.ej ->> 'step4_approved_at',
        'dispute_status',     esc.ej ->> 'dispute_status',
        'dispute_reason',     esc.ej ->> 'dispute_reason',
        'disputed_at',        esc.ej ->> 'disputed_at',
        'expected_end_date',  esc.ej ->> 'expected_end_date',
        'created_at',         esc.created_at,
        'updated_at',         esc.ej ->> 'updated_at'
      ) end,
    ...
    left join lateral (
      select e.*, to_jsonb(e) as ej from public.escrow_payments e
