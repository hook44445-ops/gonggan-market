-- ════════════════════════════════════════════════════════════════════
-- 055_admin_project_flow_drop_all_recreate.sql
-- admin_project_flow_list — 모든 overload DROP 후 깨끗한 정의로 재생성.
--
-- 배경:
--   운영 DB 의 admin_project_flow_list 함수 본문에 r.updated_at 참조가 남아
--   (position('r.updated_at' in pg_get_functiondef) = 2793),
--     column r.updated_at does not exist
--   로 GPS 흐름관리가 실패했다. requests 에 updated_at 컬럼이 없기 때문.
--   (이전 054 CREATE OR REPLACE 가 시그니처 불일치 등으로 운영 함수를 교체하지
--    못한 정황 — 잔존 버전 1개가 그대로 남음.)
--
-- 확실한 정합화(시그니처 무관하게 잔존 제거):
--   · proname='admin_project_flow_list' 인 모든 overload 를 동적으로 DROP.
--   · 단일 깨끗한 정의를 새로 CREATE — r.updated_at 미참조(0 보장).
--   · 시그니처: (text, timestamptz, timestamptz, int, text, text)
--               = (p_admin_id, p_date_from, p_date_to, p_limit, p_search, p_status).
--   · r.urgent → false, b.selected 미사용(r.selected_*), r.updated_at → r.created_at.
--   · requests 컬럼 추가 없음. grant 유지. 조회 전용.
--
-- 검증(실행 후 0 이어야 함):
--   select position('r.updated_at' in pg_get_functiondef(p.oid))
--   from pg_proc p join pg_namespace n on n.oid=p.pronamespace
--   where n.nspname='public' and p.proname='admin_project_flow_list';
--
-- 멱등 · Supabase SQL Editor 에서 1회 실행(054 이후).
-- ════════════════════════════════════════════════════════════════════
