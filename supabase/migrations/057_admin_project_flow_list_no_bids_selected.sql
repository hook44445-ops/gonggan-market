-- ════════════════════════════════════════════════════════════════════
-- 057_admin_project_flow_list_no_bids_selected.sql
-- admin_project_flow_list — 운영 DB 구조 반영(b.selected 컬럼 부재) 수정.
--
-- 문제:
--   GPS 흐름관리 진입 시 "column b.selected does not exist".
--   운영 DB bids 테이블에 selected 컬럼이 없음.
--
-- 수정(최소·RPC 내부 SQL만):
--   · b.selected 참조 전부 제거.
--   · 선택 입찰 판단 기준을 requests 기준으로 교체:
--       b.id = r.selected_bid_id  OR  b.company_id = r.selected_company_id
--   · 정렬도 b.selected desc 제거(선택 우선순위 case + created_at 만 사용).
--   · 출력 'selected' 값은 위 기준으로 계산.
--   · 그 외 로직(단계 계산/조인/필터)·시그니처·이름 변경 없음.
--
-- bids 테이블/컬럼/프론트/입찰·업체선택 로직 변경 없음. create or replace 만.
-- Supabase SQL Editor 에서 1회 실행(056 이후).
-- ════════════════════════════════════════════════════════════════════
  ...
  from (
    select jsonb_build_object(
      ...
      'selected_bid', case when sb.id is null then null else jsonb_build_object(
        ...
        'selected', (sb.id = r.selected_bid_id or sb.company_id = r.selected_company_id),
    ...
    left join public.companies comp on comp.id = coalesce(
      ...
      -- 선택 입찰의 company_id — 운영 DB 기준(b.selected 컬럼 없음):
      --   1순위 r.selected_bid_id, 2순위 r.selected_company_id 매칭.
      (select b.company_id from public.bids b
        where b.request_id = r.id
          and (b.id = r.selected_bid_id or b.company_id = r.selected_company_id)
    ...
    left join lateral (
      -- 선택 입찰 — requests 기준(b.selected 미사용).
