-- ════════════════════════════════════════════════════════════════════
-- 059_admin_project_flow_list_no_urgent.sql
-- admin_project_flow_list — requests.urgent 컬럼 부재 대응.
--
-- 문제:
--   GPS 흐름관리 진입 시 "column r.urgent does not exist".
--   운영 DB requests 테이블에 urgent 컬럼이 없음.
--
-- 수정(RPC 내부 SQL만, 최소):
--   · 출력 'urgent' 값을 r.urgent → false 로 fallback(컬럼 참조 제거).
--   · 그 외(권한 체크 sentinel 허용, b.selected 미사용, 단계계산/조인/필터)는 058 과 동일.
--
-- requests 테이블 컬럼 추가 없음. 프론트/GPS/site_visits/project_checkpoints/
-- bids/reviews/결제/에스크로 변경 없음. create or replace 만.
-- Supabase SQL Editor 에서 1회 실행(058 이후). notify 포함.
-- ════════════════════════════════════════════════════════════════════
