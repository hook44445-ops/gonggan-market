-- ════════════════════════════════════════════════════════════════════
-- 058_admin_project_flow_list_admin_sentinel.sql
-- admin_project_flow_list — 관리자 권한 체크를 운영 앱 로그인 구조에 맞춤.
--
-- 문제:
--   GPS 흐름관리 진입 시 ADMIN_ONLY.
--   운영 앱 관리자 로그인은 가상 sentinel: user.id = 'admin', role = 'admin'.
--   프론트는 p_admin_id = user.id = 'admin' 를 전송한다(프론트 수정 불필요).
--   라이브 함수의 권한 체크가 'admin' sentinel 을 허용하지 않아 거부됨.
--
-- 수정(권한 로직만, 최소):
--   · uuid 면: users.role='admin' 필수(기존 유지).
--   · 비-uuid 면: 'admin'(트림·소문자) 일 때만 허용. 그 외 거부.
--   · 거부 시 받은 값(p_admin_id)을 에러 메시지에 노출 → 재발 시 즉시 진단.
--   · 본문(단계계산/조인/필터/b.selected 미사용)은 057 과 동일.
--
-- 테이블/프론트/users 데이터 변경 없음. create or replace 만.
-- Supabase SQL Editor 에서 1회 실행(057 이후). notify 포함.
-- ════════════════════════════════════════════════════════════════════

set search_path = public, extensions;

-- sentinel uuid 변환 헬퍼(없으면 생성, 멱등).
...
declare
  ...
  v_result     jsonb;
begin
  -- 관리자 검증 — 운영 앱 sentinel('admin') 또는 DB role=admin uuid 허용.
