-- ════════════════════════════════════════════════════════════════════
-- 054_admin_project_flow_align_prod.sql
-- admin_project_flow_list — 운영 중인 최신 RPC 기준으로 정합화.
--
-- 배경:
--   GPS 흐름관리 디버깅 중 운영 SQL Editor 에서 RPC 가 직접 수정되어
--   (PGRST202·b.selected·ADMIN_ONLY·r.urgent 해결), 시그니처/본문이 repo 의
--   047/053 과 달라졌다. 047/053 은 다음 문제를 갖는다:
--     · 시그니처 순서가 프론트 호출과 다름(구: text,text,text,ts,ts,int).
--       param 이름은 같아 신 시그니처와 공존 시 overload 모호성(PGRST203).
--     · requests.urgent 컬럼 없음 → r.urgent 참조 실패.
--     · bids.selected 컬럼 없음 → b.selected 참조 실패.
--     · requests.updated_at 컬럼 없음 → r.updated_at 참조 실패.
--
-- 이 마이그레이션(운영 최신본 기준, r.updated_at 만 r.created_at 으로):
--   · 시그니처: (p_admin_id text, p_date_from timestamptz, p_date_to timestamptz,
--               p_limit int, p_search text, p_status text)  ← 운영/프론트 호출과 일치.
--   · 구 시그니처(text,text,text,ts,ts,int) DROP — overload 모호성 제거.
--   · r.urgent  → false        (requests.urgent 없음)
--   · b.selected → 미사용       (선택 업체/입찰은 r.selected_company_id / r.selected_bid_id 기준)
--   · r.updated_at → r.created_at (출력·정렬 모두)
--   · 조회 전용 · 결제/정산/직거래 로직 변경 없음.
--
-- 멱등 · Supabase SQL Editor 에서 1회 실행(053 이후).
-- ════════════════════════════════════════════════════════════════════

set search_path = public, extensions;

-- 구 시그니처(047 순서) 제거 — 신 시그니처와 param 이름이 같아 모호성 유발.
