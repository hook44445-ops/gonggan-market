...
export default function EscrowScreen({ onBack, mode, selectedBid }) {
  ...
  const handleFileChange = (e) => {
    ...
    setIsUploading(true);
    setTimeout(() => {
      const previews = files.map((f, i) => i < PLACEHOLDER_PHOTOS.length ? PLACEHOLDER_PHOTOS[i] : PLACEHOLDER_PHOTOS[0]);
  ...
  const handleRemovePhoto = (idx) => setUploadedPhotos(prev => prev.filter((_, i) => i !== idx));

  const handleUploadSubmit = () => {
    setIsUploading(true);
    setTimeout(() => { setIsUploading(false); setUploadDone(true); }, 1500);
  };

  const photosToShow = uploadDone ? uploadedPhotos : PLACEHOLDER_PHOTOS;

  // Merge ESCROW_STEPS with computed stage amounts
  const stepsWithAmounts = ESCROW_STEPS.map((step, i) => {
    const stage = stages[i - 1]; // ESCROW_STEPS[0] = deposit event (no stage calc)
    return { ...step, stage };
  });
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Pretendard','Apple SD Gothic Neo',sans-serif" }}>
      ...
      <div style={{ padding:`${S.xl}px ${S.xl}px 40px` }}>
        ...
        {/* Role banner — fee display is role-specific, never mixed */}
        ...
        {/* Amount card — consumer sees total incl. 3% fee; company sees bid amount */}
        <div style={{ background:`linear-gradient(135deg,${C.navy},${C.navyM})`,
          ...
          {isConsumer ? (
            ...
            <>
              ...
              <div style={{ fontSize:13, opacity:0.75, marginBottom:S.xl }}>고객 예치 완료 · 단계별로 입금됩니다</div>
            </>
          )}
          ...
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, opacity:0.7 }}>
            ...
            {bidAmount > 0 && (
              <span>보관 중 {fmtMoney(Math.round(bidAmount * (100 - paid) / 100))}</span>
            )}
          </div>
        </div>
        ...
        <div style={{ background:C.surface, borderRadius:R.xl, padding:S.xl, marginBottom:S.xl, border:`1px solid ${C.bgWarm}` }}>
          ...
          {stepsWithAmounts.map((s, i) => (
            <div key={s.id} style={{ display:"flex", gap:S.md, marginBottom:i < stepsWithAmounts.length - 1 ? S.xl : 0 }}>
              <div style={{ display:"flex", flexDirection:"column", alignItems:"center", flexShrink:0 }}>
                ...
                {i < stepsWithAmounts.length - 1 && (
                  <div style={{ width:2, flex:1, minHeight:20, marginTop:4, background:s.done ? C.green : C.bgWarm }} />
                )}
              </div>
              <div style={{ flex:1, paddingTop:8 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:3 }}>
                  <div style={{ fontSize:14, fontWeight:800, color:s.done ? C.green : s.active ? C.brand : C.text3 }}>{s.label}</div>
                  {s.stage && (
                    <div style={{ fontSize:13, fontWeight:700, color:s.done ? C.green : s.active ? C.brand : C.text4, textAlign:"right" }}>
                      {/* Consumer sees stage amount; company sees receive amount after 4% fee */}
                      {isConsumer
                        ? fmtMoney(s.stage.amount)
                        : (
                          <span>
                            {fmtMoney(s.stage.amount)}
                            <span style={{ fontSize:11, marginLeft:4, color:s.done ? C.green : s.active ? C.brand : C.text4 }}>
                              {" →실수령 "}{fmtMoney(s.stage.companyReceiveAmount)}
                            </span>
                          </span>
                        )
                      }
                      {!isConsumer && (
                        <span style={{ fontSize:11, display:"block", color:s.done ? C.green : s.active ? C.brand : C.text4 }}>
                ...
                {s.active && !isConsumer && !confirmed && (
                  <div style={{ background:C.surface2, borderRadius:R.lg, padding:S.lg, border:`1px solid ${C.bgWarm}` }}>
                    ...
                    <div style={{ fontSize:12, color:C.text3, lineHeight:1.6, marginBottom:S.md }}>
                      고객이 사진을 확인하면 중도금 {s.stage ? fmtMoney(s.stage.amount) : "40%"} 지급 승인이 진행됩니다.
                    </div>
                    {uploadDone ? (
                      <>
                        ...
                        <div style={{ display:"flex", gap:S.sm }}>
                          <button onClick={() => fileInputRef.current?.click()} style={{ flex:1, padding:"10px", background:C.surface, color:C.text2, border:`1px solid ${C.bgWarm}`, borderRadius:R.lg, fontWeight:700, fontSize:13, cursor:"pointer" }}>+ 추가</button>
                          <div style={{ flex:2, background:C.greenL, borderRadius:R.lg, padding:"10px", display:"flex", alignItems:"center", justifyContent:"center", gap:S.sm }}>
                            <span style={{ fontSize:14 }}>✅</span>
                            <span style={{ fontSize:13, fontWeight:700, color:C.green }}>업로드 완료 · 고객 확인 대기중</span>
                          </div>
                        </div>
                      </>
                    ) : (
                      <>
                          ...
                          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:S.sm, marginBottom:S.md }}>
                            {uploadedPhotos.map((src, pi) => (
                              <div key={pi} style={{ position:"relative", aspectRatio:"1", borderRadius:R.md, overflow:"hidden", border:`1px solid ${C.bgWarm}` }}>
                                ...
                                <button onClick={() => handleRemovePhoto(pi)} style={{ position:"absolute", top:4, right:4, width:20, height:20, background:"rgba(0,0,0,0.55)", color:"#fff", border:"none", borderRadius:R.full, fontSize:11, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
                              </div>
                            ))}
                        ...
                        <div style={{ display:"flex", gap:S.sm }}>
                          <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} style={{ flex:1, padding:"11px", background:C.surface, color:C.text2, border:`1px solid ${C.bgWarm}`, borderRadius:R.lg, fontWeight:700, fontSize:13, cursor:"pointer" }}>📁 사진 선택</button>
                          <button onClick={handleUploadSubmit} disabled={isUploading || uploadedPhotos.length === 0}
                            style={{ flex:2, padding:"11px", borderRadius:R.lg, fontWeight:800, fontSize:14, cursor:uploadedPhotos.length > 0 ? "pointer" : "not-allowed", border:"none",
                              background:uploadedPhotos.length > 0 ? C.brand : C.bgWarm,
                              color:uploadedPhotos.length > 0 ? "#fff" : C.text4,
                              boxShadow:uploadedPhotos.length > 0 ? `0 4px 14px ${C.brand}44` : "none" }}>
                ...
                {/* Consumer: photo confirm — only consumer-facing fee info shown here */}
                {s.active && isConsumer && !confirmed && (
                  <div style={{ background:C.brandL, borderRadius:R.lg, padding:S.lg, border:`1px solid ${C.brandM}` }}>
                    ...
                    <div style={{ display:"flex", gap:S.sm }}>
                      <button onClick={() => setShowDisputeSheet(true)} style={{ flex:1, padding:"11px", background:C.surface, color:C.red, border:`1px solid ${C.red}33`, borderRadius:R.lg, fontWeight:700, fontSize:13, cursor:"pointer" }}>⚠️ 이의 신청</button>
                      <button onClick={() => setShowConfirm(true)} style={{ flex:2, padding:"11px", background:C.brand, color:"#fff", border:"none", borderRadius:R.lg, fontWeight:800, fontSize:13, cursor:"pointer", boxShadow:`0 4px 14px ${C.brand}44` }}>✅ 확인 · 중도금 승인</button>
                    </div>
                  </div>
                )}
                ...
                {s.active && confirmed && (
                  <div style={{ background:C.greenL, borderRadius:R.lg, padding:S.md, display:"flex", alignItems:"center", gap:S.sm }}>
                    ...
                    <span style={{ fontSize:13, color:C.green, fontWeight:700 }}>{isConsumer ? "승인 완료 · 중도금 지급됨" : "중도금 입금 완료"}</span>
                  </div>
                )}
                {s.active && disputeSubmitted && !confirmed && (
                  <div style={{ background:"#FFF0F0", borderRadius:R.lg, padding:S.md, display:"flex", alignItems:"center", gap:S.sm, border:`1px solid ${C.red}22` }}>
          ...
          <div style={{ background:C.surface, borderRadius:"24px 24px 0 0", width:"100%", maxWidth:480, padding:"24px 24px 40px" }}>
            ...
            <div style={{ textAlign:"center", marginBottom:S.xxl }}>
              ...
              <div style={{ fontSize:13, color:C.text3, lineHeight:1.6 }}>
                공간마켓이 보관 중인 금액에서<br/>
                <b style={{color:C.text1}}>{fmtMoney(stages[1].amount)}</b>을 업체에 지급합니다
              </div>
            </div>
      ...
      {showDisputeSheet && (
        <div style={{ position:"fixed", inset:0, background:"rgba(31,42,36,0.65)", display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:300 }} onClick={e => { if (e.target === e.currentTarget) setShowDisputeSheet(false); }}>
          <div style={{ background:C.surface, borderRadius:"24px 24px 0 0", width:"100%", maxWidth:480, padding:"24px 24px 40px" }}>
            ...
            <div style={{ fontSize:13, color:C.text3, lineHeight:1.6, marginBottom:S.xl }}>사진의 시공 상태가 계약 내용과 다를 경우 이의를 신청하세요.<br/>공간마켓 중재팀이 검토 후 연락드립니다.</div>
            ...
            <div style={{ display:"flex", gap:S.sm }}>
              <button onClick={() => setShowDisputeSheet(false)} style={{ flex:1, padding:S.xl, background:C.bg, color:C.text2, border:`1px solid ${C.bgWarm}`, borderRadius:R.lg, fontWeight:700, fontSize:15, cursor:"pointer" }}>취소</button>
              <button onClick={() => { if (!disputeReason.trim()) return; setShowDisputeSheet(false); setDisputeSubmitted(true); }}
