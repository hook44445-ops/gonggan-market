...
export default function ReviewModal({ onClose, onSubmit }) {
  ...
  const delta      = calcTempDelta(rating, hasPhoto);
      ...
      <div style={{ background:C.surface, borderRadius:"24px 24px 0 0",
        ...
        {step === 2 && <>
          ...
          <div onClick={() => setHasPhoto(p => !p)}
            style={{ display:"flex", alignItems:"center", gap:S.sm, padding:`${S.md}px ${S.lg}px`,
              background:hasPhoto?C.brandL:C.surface2, borderRadius:R.lg,
              border:`1.5px solid ${hasPhoto?C.brandM:C.bgWarm}`,
              cursor:"pointer", marginBottom:S.md }}>
            <div style={{ width:20, height:20, borderRadius:4,
              border:`2px solid ${hasPhoto?C.brand:C.text4}`,
              background:hasPhoto?C.brand:"transparent",
              display:"flex", alignItems:"center", justifyContent:"center",
              fontSize:12, color:"#fff", flexShrink:0 }}>{hasPhoto ? "✓" : ""}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:13, fontWeight:700, color:hasPhoto?C.brand:C.text2 }}>📷 사진 포함</div>
              <div style={{ fontSize:11, color:C.text3, marginTop:1 }}>사진 첨부 시 공간온도 +0.3° 추가 상승</div>
            </div>
          </div>
