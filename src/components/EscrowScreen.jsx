...
function CountdownTimer({ deadlineMs }) {
  ...
  return (
    <div style={{ background:C.brandL, borderRadius:R.lg, padding:S.lg,
      border:`1px solid ${C.brandM}`, marginBottom:S.md }}>
      ...
      <div style={{ background:`${C.brand}22`, borderRadius:R.full, height:6 }}>
        <div style={{ width:`${pct}%`, height:"100%", background:C.brand,
          borderRadius:R.full, transition:"width 1s linear" }} />
      </div>
      <div style={{ fontSize:11, color:C.text3, marginTop:S.xs }}>
...
export default function EscrowScreen({ onBack, mode }) {
  ...
  // Company photo upload state
  const [uploadedPhotos, setUploadedPhotos] = useState([]);
  const [isUploading, setIsUploading] = useState(false);
  const [uploadDone, setUploadDone] = useState(false);
  const fileInputRef = useRef(null);

  // Consumer confirm state
  ...
  // 72hr deadline: in demo mode, set to 71h from now so countdown shows
  const [deadline] = useState(() => Date.now() + 71 * 3600 * 1000 + 59 * 60 * 1000);

  const paid = ESCROW_STEPS.filter(s => s.done).reduce((a, s) => a + s.pct, 0);

  // Simulate file selection → preview
  const handleFileChange = (e) => {
    const files = Array.from(e.target.files);
    if (!files.length) return;
    setIsUploading(true);
    setTimeout(() => {
      // Use demo photos as previews (browser FileReader would work for real files)
      const previews = files.map((f, i) =>
        i < PLACEHOLDER_PHOTOS.length ? PLACEHOLDER_PHOTOS[i] : PLACEHOLDER_PHOTOS[0]
      );
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Apple SD Gothic Neo',sans-serif" }}>
      ...
      <div style={{ background:C.surface, padding:"14px 20px", borderBottom:`1px solid ${C.bgWarm}`,
        ...
        <div style={{ marginLeft:"auto", background:C.navyL, borderRadius:R.full,
          padding:"4px 12px", fontSize:12, fontWeight:700, color:C.navy }}>🛡 보호중</div>
      </div>
      ...
      <div style={{ padding:`${S.xl}px ${S.xl}px 40px` }}>
        ...
        <div style={{ background:`linear-gradient(135deg,${C.navy},${C.navyM})`,
          ...
          <div style={{ background:"rgba(255,255,255,0.15)", borderRadius:R.full, height:8, marginBottom:6 }}>
            <div style={{ width:`${paid}%`, height:"100%", background:C.brand,
              borderRadius:R.full, transition:"width 0.6s ease",
              boxShadow:`0 0 8px ${C.brand}88` }} />
          </div>
          <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, opacity:0.7 }}>
            <span>업체 지급 완료 {paid}%</span>
            <span>보관 중 {Math.round(total * (100 - paid) / 100).toLocaleString()}만원</span>
          </div>
        </div>
          ...
          {ESCROW_STEPS.map((s, i) => (
            <div key={s.id} style={{ display:"flex", gap:S.md, marginBottom:i < ESCROW_STEPS.length - 1 ? S.xl : 0 }}>
              ...
              <div style={{ flex:1, paddingTop:8 }}>
                ...
                <div style={{ fontSize:12, color:C.text3, lineHeight:1.5, marginBottom:s.active ? S.md : 0 }}>
                  {s.sub}
                </div>

                {/* ── Company: photo upload section ── */}
                      ...
                      <>
                        ...
                        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:S.sm, marginBottom:S.md }}>
                          {uploadedPhotos.map((src, pi) => (
                            <div key={pi} style={{ position:"relative", aspectRatio:"1", borderRadius:R.md,
                              overflow:"hidden", border:`1px solid ${C.bgWarm}` }}>
                              <img src={src} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }}
                                onError={e => { e.target.style.background = C.bgWarm; }} />
                              <button onClick={() => setUploadedPhotos(prev => prev.filter((_,i)=>i!==pi))}
                                style={{ position:"absolute", top:4, right:4, width:20, height:20,
                                  background:"rgba(0,0,0,0.55)", color:"#fff", border:"none",
                                  borderRadius:R.full, fontSize:11, cursor:"pointer",
                        ...
                        <div style={{ display:"flex", gap:S.sm }}>
                          <button onClick={() => fileInputRef.current?.click()}
                            style={{ flex:1, padding:"10px", background:C.surface, color:C.text2,
                              border:`1px solid ${C.bgWarm}`, borderRadius:R.lg,
                              fontWeight:700, fontSize:13, cursor:"pointer" }}>
                            + 추가
                          </button>
                          <div style={{ flex:2, background:C.greenL, borderRadius:R.lg, padding:"10px",
                      ...
                      <>
                          ...
                          <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:S.sm, marginBottom:S.md }}>
                            {uploadedPhotos.map((src, pi) => (
                              <div key={pi} style={{ position:"relative", aspectRatio:"1", borderRadius:R.md,
                                overflow:"hidden", border:`1px solid ${C.bgWarm}` }}>
                                <img src={src} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} />
                                <button onClick={() => handleRemovePhoto(pi)}
                                  style={{ position:"absolute", top:4, right:4, width:20, height:20,
                                    background:"rgba(0,0,0,0.55)", color:"#fff", border:"none",
                                    borderRadius:R.full, fontSize:11, cursor:"pointer",
                                    display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
                            ...
                            {uploadedPhotos.length < 6 && (
                              <button onClick={() => fileInputRef.current?.click()}
                                style={{ aspectRatio:"1", background:C.bg, border:`2px dashed ${C.bgWarm}`,
                                  borderRadius:R.md, cursor:"pointer", display:"flex",
                        ...
                        {uploadedPhotos.length === 0 && (
                          <button onClick={() => fileInputRef.current?.click()}
                            ...
                            style={{ width:"100%", padding:"18px", background:C.bg,
                              border:`2px dashed ${C.bgWarm}`, borderRadius:R.lg,
                              display:"flex", flexDirection:"column", alignItems:"center",
                              justifyContent:"center", gap:S.sm, cursor:"pointer", marginBottom:S.md }}>
                            <span style={{ fontSize:28 }}>{isUploading ? "⏳" : "📷"}</span>
                            <span style={{ fontSize:13, color:C.text3, fontWeight:600 }}>
                              {isUploading ? "처리 중..." : "사진을 선택하세요"}
                            </span>
                        ...
                        <div style={{ display:"flex", gap:S.sm }}>
                          <button onClick={() => fileInputRef.current?.click()}
                            disabled={isUploading}
                            style={{ flex:1, padding:"11px", background:C.surface, color:C.text2,
                              border:`1px solid ${C.bgWarm}`, borderRadius:R.lg,
                              fontWeight:700, fontSize:13, cursor:"pointer" }}>
                            📁 사진 선택
                          </button>
                          <button
                            ...
                            style={{ flex:2, padding:"11px", borderRadius:R.lg, fontWeight:800, fontSize:14,
                              cursor:uploadedPhotos.length > 0 ? "pointer" : "not-allowed", border:"none",
                ...
                {/* ── Consumer: photo confirm section ── */}
                {s.active && isConsumer && !confirmed && (
                  <div style={{ background:C.brandL, borderRadius:R.lg, padding:S.lg,
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:S.sm, marginBottom:S.md }}>
                      {photosToShow.map((src, pi) => (
                        <div key={pi} style={{ borderRadius:R.md, overflow:"hidden",
                          border:`1px solid ${C.brandM}`, aspectRatio:"4/3" }}>
                          <img src={src} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }}
                            onError={e => { e.target.style.background = C.bgWarm; }} />
                        </div>
                      ))}
                    ...
                    <div style={{ display:"flex", gap:S.sm }}>
                      ...
                      <button onClick={() => setShowConfirm(true)}
                        style={{ flex:2, padding:"11px", background:C.brand, color:"#fff",
                ...
                {/* Dispute submitted state */}
                {s.active && disputeSubmitted && !confirmed && (
                  <div style={{ background:"#FFF0F0", borderRadius:R.lg, padding:S.md,
                    display:"flex", alignItems:"center", gap:S.sm, border:`1px solid ${C.red}22` }}>
        ...
        <div style={{ background:C.navyL, borderRadius:R.xl, padding:S.xl,
          border:`1px solid ${C.trustM}`, display:"flex", gap:S.md,
          ...
          <div>
            ...
            <div style={{ fontSize:12, color:C.text3, lineHeight:1.7 }}>
              완료 확인 후 <b style={{color:C.navy}}>1년간 무상 AS</b> 보장
            </div>
          </div>
        </div>
          ...
          <div style={{ background:C.surface, borderRadius:"24px 24px 0 0", width:"100%",
            ...
            <div style={{ textAlign:"center", marginBottom:S.xxl }}>
              ...
              <div style={{ fontSize:13, color:C.text3, lineHeight:1.6 }}>
                공간마켓이 보관 중인 금액에서<br/>
                <b style={{color:C.text1}}>{Math.round(total * 0.4).toLocaleString()}만원</b>을 업체에 지급합니다
              </div>
            </div>
            <div style={{ background:C.surface2, borderRadius:R.lg, padding:S.lg, marginBottom:S.xl }}>
              {[
                ["선금 (30%)",  Math.round(total*0.3).toLocaleString()+"만원", true ],
                ["중도금 (40%)",Math.round(total*0.4).toLocaleString()+"만원", false],
                ["잔금 (30%)",  Math.round(total*0.3).toLocaleString()+"만원", false],
              ].map(([l, v, isPaid], i) => (
                <div key={l} style={{ display:"flex", justifyContent:"space-between",
                  padding:`${S.sm}px 0`, borderBottom:i<2?`1px solid ${C.bgWarm}`:"none" }}>
                  <span style={{ fontSize:13, color:isPaid?C.text3:C.text2, fontWeight:600 }}>{l}</span>
                  <span style={{ fontSize:13, fontWeight:700, color:isPaid?C.text4:C.text1,
                    textDecoration:isPaid?"line-through":"none" }}>{v}</span>
                </div>
              ))}
            </div>
            <div style={{ display:"flex", gap:S.sm }}>
              ...
              <button onClick={() => { setShowConfirm(false); setConfirmed(true); }}
      ...
      {showDisputeSheet && (
        <div style={{ position:"fixed", inset:0, background:"rgba(28,23,18,0.6)",
          display:"flex", alignItems:"flex-end", justifyContent:"center", zIndex:300 }}
          onClick={e => { if (e.target === e.currentTarget) setShowDisputeSheet(false); }}>
          <div style={{ background:C.surface, borderRadius:"24px 24px 0 0", width:"100%",
            maxWidth:480, padding:"24px 24px 40px" }}>
            <div style={{ width:36, height:4, background:C.bgWarm, borderRadius:R.full, margin:"0 auto 20px" }} />
            ...
            <div style={{ fontSize:13, color:C.text3, lineHeight:1.6, marginBottom:S.xl }}>
              사진의 시공 상태가 계약 내용과 다를 경우 이의를 신청하세요.<br/>
              공간마켓 중재팀이 검토 후 연락드립니다.
            </div>
            <textarea
              value={disputeReason}
              ...
              style={{ width:"100%", padding:S.lg, borderRadius:R.lg, border:`1px solid ${C.bgWarm}`,
                background:C.surface2, fontSize:13, color:C.text1, resize:"none", height:100,
                boxSizing:"border-box", marginBottom:S.xl, outline:"none",
                fontFamily:"inherit", lineHeight:1.6 }}
            />
            <div style={{ display:"flex", gap:S.sm }}>
              <button onClick={() => setShowDisputeSheet(false)}
                style={{ flex:1, padding:S.xl, background:C.bg, color:C.text2,
                  border:`1px solid ${C.bgWarm}`, borderRadius:R.lg, fontWeight:700, fontSize:15, cursor:"pointer" }}>
                취소
              </button>
              <button
                ...
                style={{ flex:2, padding:S.xl,
                  background:disputeReason.trim() ? C.red : C.bgWarm,
                  color:disputeReason.trim() ? "#fff" : C.text4,
