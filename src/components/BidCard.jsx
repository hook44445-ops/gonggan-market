...
export default function BidCard({
  ...
  // ── 업체 선정 상태 판별 ────────────────────────────────────────────────
  // 아래 중 하나라도 true 이면 선정 이후 → "의뢰인 검토 중 / 입찰 수정" UI 숨김
  const isChosen =
    r?.status === "in_progress" ||
    r?.status === "selected" ||
    (myBid?.id && r?.selectedBidId && r.selectedBidId === myBid.id) ||
    (currentUser?.id && r?.selectedCompanyId && r.selectedCompanyId === currentUser.id) ||
    myBid?.status === "selected" ||
    siteVisit?.status === "requested" ||
    siteVisit?.status === "accepted";

  const isSiteVisitRequested = siteVisit?.status === "requested";
  const isSiteVisitAccepted  = siteVisit?.status === "accepted";
            ...
            <div key="reviewing" style={{ background: C.greenL, borderRadius: R.lg, padding: S.lg, border: `1px solid ${C.green}33` }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
                ...
                {!isClosed && !isGuest && (
                  <button
                    onClick={openEdit}
                    style={{ background: C.surface, color: C.brand, border: `1px solid ${C.brandM}`, borderRadius: R.full, padding: "5px 14px", fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
                    ✏️ 입찰 수정
                  </button>
                )}
              </div>
