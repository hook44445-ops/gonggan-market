      ...
      <div style={box}>
        ...
        {readyDrafts.length === 0 ? <div style={{ fontSize: 11.5, color: C.text3 }}>예약할 초안이 없습니다(초안 상태 글).</div> : readyDrafts.map((d) => {
          ...
          <div key={d.id} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11.5, padding: "5px 0", borderBottom: `1px solid ${C.bg}`, flexWrap: "wrap" }}>
            ...
            {b && <span title={`규칙 기반 사전검사(휴리스틱) · 실제 AI 검수 미실행 · ${b.reviewers.map((r) => `${r.role}:${r.decision}`).join(" · ")}`} style={{ padding: "2px 8px", borderRadius: R.full, fontSize: 9.5, fontWeight: 800, background: bColor + "22", color: bColor, whiteSpace: "nowrap" }}>사전검사 {b.approvalCount}/4·게이트 {b.hardGatePassed ? "PASS" : "FAIL"}</span>}
