...
export function analyzeStrategy(issue, { categoryHint = null } = {}) {
  const t = norm(issue);
  ...
  const isNews = breaking || (area && NEWS_AREAS.has(area.id)) || NEWS_CATEGORIES.has(category);
  const isStory = STORY_HINTS.some((h) => t.includes(norm(h)));
  // 공간 연결: 재해석 렌즈가 기본(life)이 아니고, 생활형 카테고리이면 공간관점이 자연스럽다.
  const spaceConnected = reinterpret.lensId !== "life" && SPACE_LIFE_CATEGORIES.has(category);
  ...
  const chain = [catLabel, isNews ? "뉴스" : isStory ? "연재" : isColumn ? "칼럼" : "일반", modeLabel, promptVersion, `Temp ${temperature}`];
  const reason = `${catLabel} · ${isNews ? "객관 데이터형" : isStory ? "연재형" : spaceConnected ? "공간 연결형" : "칼럼형"} → ${modeLabel}(${promptVersion}, ${temperature})`;

  return { mode, promptVersion, temperature, category, categoryLabel: catLabel, style, isNews, isStory, spaceConnected, modeLabel, chain, reason };
}
