...
import { LOUNGE_INACTIVE_CATEGORIES } from '../constants/lounge';
...
function adaptSeedPost(s) {
  return {
    ...
    is_seed:              true,
    has_badge:            false,
    // 077: 시딩 DB 값을 그대로 전달(region 고정 null 제거). 전문가 표시 확장 필드 포함.
    region:               s.region ?? null,
    seed_type:            s.seed_type ?? null,
    is_recommended:       s.is_recommended === true,
    is_expert:            s.is_expert === true,
    expert_company_name:  s.expert_company_name ?? null,
    expert_badge:         s.expert_badge ?? null,
    expert_job:           s.expert_job ?? null,
    gender:               null,
    age_group:            null,
  ...
  const loadPosts = useCallback(async () => {
    ...
    if (IS_SUPABASE_READY) {
      ...
      const merged    = buildFeed(realPosts, seeds)
        ...
        // 비활성 카테고리(게임/반려동물/여행/대화해요) 글 노출 제외 — 전체 탭 포함
