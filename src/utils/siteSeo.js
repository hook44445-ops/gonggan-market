  ...
  return [
    {
      ...
      a: '어떤 공간을 어디까지 고칠지 한 번만 적으면 됩니다. 업체들이 같은 조건을 보고 견적을 보내고, 금액·공정·기간을 나란히 놓고 비교합니다. 업체가 사업자등록·시공보험·보증금을 어디까지 증빙했는지도 카드에서 함께 보입니다.',
    },
...
export function pageSeo(beta) {
  return {
    '/': {
      ...
      //
      // ⚠️ 「사업자등록을 확인한 업체가 견적을 보낸다」고 쓰면 안 된다(2026-09-24 수정).
      //    새 정책에서는 가입만 한 업체도 300만원까지 입찰한다 — 사실과 다르고,
      //    같은 페이지 FAQ(「증빙이 적은 업체는 작은 공사만」)와도 모순된다.
      //    이 문장은 title·description·OG·프리렌더·llms.txt 에 모두 실려 색인된다.
      title: '인테리어 비교견적 — 공간마켓 · 집수리 리모델링 견적',
      description: beta
        ? '인테리어 견적, 여러 곳을 나란히 놓고 비교하세요. 업체가 사업자등록·시공보험·보증금을 어디까지 증빙했는지 보이고, 공사 사진과 대화가 기록으로 남습니다. 견적 요청 무료.'
        : '인테리어 견적, 여러 곳을 나란히 놓고 비교하세요. 업체가 어디까지 증빙했는지 보이고, 공간안전결제(에스크로)로 단계마다 확인한 뒤 지급합니다. 견적 요청 무료.',
