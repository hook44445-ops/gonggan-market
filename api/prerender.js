...
<section>
<h2>인테리어 업체이신가요?</h2>
<p>공간마켓 공간파트너는 광고비·월정액 없이, 계약이 성사된 프로젝트에만 ${esc(PARTNER_FEE_RATE)}(VAT 포함) 이용수수료가 발생합니다.</p>
...
<section>
<h2>비용은 얼마인가요?</h2>
<p>가입비·광고비·월정액이 없습니다. 견적 요청을 받는 것도 무료이며, 계약이 성사된 프로젝트에만 ${esc(PARTNER_FEE_RATE)}(VAT 포함)의 이용수수료가 발생합니다. 수주에 실패하면 어떤 비용도 청구되지 않습니다.</p>
</section>

<section>
<h2>예치보증금 등급과 수주 한도</h2>
<p>예치보증금은 가입비가 아니라 신뢰를 보증하는 예치금이며, 활동 종료 시 100% 환급됩니다. 예치 금액에 따라 수주 가능 금액 한도가 정해집니다.</p>
<ul>${PARTNER_GRADES.map((g) => `<li>${esc(g.name)} — 보증금 ${esc(g.deposit)} · 수주 한도 ${esc(g.limit)}</li>`).join('')}</ul>
</section>
...
function renderLlms(req, res, site) {
  ...
  const lines = [
    ...
    beta
      ...
      : `- 공간안전결제(에스크로)로 시공대금을 예치하고, 단계 확인 후 ${ESCROW_STAGES.map(([n, , p]) => `${n} ${p}`).join(', ')} 비율로 지급합니다.`,
    '',
    '## 시공 업체(공급자)가 받는 것',
    '- 가입비·광고비·월정액이 없습니다. 견적 요청 수신도 무료입니다.',
    `- 계약이 성사된 프로젝트에만 ${PARTNER_FEE_RATE}(VAT 포함) 이용수수료가 발생합니다.`,
    '- 수주에 실패하면 어떤 비용도 청구되지 않습니다.',
    '- 예치보증금은 가입비가 아니며 활동 종료 시 100% 환급됩니다. 예치 금액에 따라 수주 한도가 정해집니다:',
    ...PARTNER_GRADES.map((g) => `  - ${g.name}: 보증금 ${g.deposit}, 수주 한도 ${g.limit}`),
    '',
    '## 자주 묻는 질문 (의뢰인)',
